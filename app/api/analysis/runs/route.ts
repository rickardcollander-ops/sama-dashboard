    ...
    try {
      ...
      if (upstream.ok) {
        ...
        if (Array.isArray(data?.runs)) {
          // Filter by domain only when we know the workspace's domain.
          // When expectedDomain is empty, trust the upstream's tenant
          // scoping and pass rows through.
          backendRuns = expectedDomain
            ? (data.runs as RunSummary[]).filter((r) =>

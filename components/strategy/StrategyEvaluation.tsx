...
export default function StrategyEvaluation({ tenantId }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Kunde inte hämta utvärdering");
    }
    setLoading(false);
  };
    ...
    <section className="rounded-xl border bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Hur har de topics du valt i strategin presterat? Mäts mot AI-omnämnanden,
            Google-position och publicerat content.
          </p>
        </div>
        <button
          onClick={load}
      ...
      {data && (data.topics?.length ?? 0) === 0 && (
        <div className="mt-4 rounded-lg border border-dashed border-slate-200 bg-slate-50/50 px-3 py-4 text-sm text-slate-500">
          Inga topics att mäta än. Generera en strategi först — vi knyter sedan an din
          AI-synlighet, SEO-data och content till varje topic.
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 text-left text-xs text-slate-500">
                ...
                <th className="px-3 py-2 font-medium">AI-omnämnande</th>
                ...
                return (
                  <tr key={i} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2 align-top text-slate-700">
                      ...
                      {t.ai_checks ? (
                        <span className="text-xs text-slate-400"> · {t.ai_checks} kontroller</span>
                      ) : null}
                    </td>
                    <td className="px-3 py-2 align-top text-slate-700">
                      {t.seo_avg_position ? (
                        <>
                          ...
                          <span className="text-xs text-slate-400"> · {t.seo_keywords ?? 0} sökord</span>
                        </>
                      ) : (
                        ...
                        <>
                          ...
                          {(t.content_published ?? 0) > 0 && (
                            <span className="text-xs text-emerald-600">
                              {" "}
                              · {t.content_published} publicerat
                            </span>
                          )}
                        </>
                      ) : (
      ...
      {data && (data.topics?.length ?? 0) > 0 && (
        <div className="mt-4 flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50/60 p-3 text-xs text-slate-500">
          ...
          <span>
            <strong className="text-slate-700">Tips:</strong> Topics i &quot;Bakåt&quot; behöver
            content som svarar mot frågan. Klicka{" "}
            <Link
              href="/c/content"
            ...
            och föreslå med AI — gap-listan är din ingång.
          </span>
        </div>
      )}
    </section>

...
function buildAuthHeader(cfg: Record<string, string>): string {
  ...
  if (!user || !pass) {
    throw new PublishError("Duda: api_user and api_pass are required", 400);
  }
...
export const dudaAdapter: CmsAdapter = {
  ...
  async validate(cfg) {
    try {
      ...
      if (!site) return { ok: false, message: "site_name is required" };
  ...
  async publish(cfg, input: PublishInput): Promise<PublishResult> {
    const site = cfg.site_name?.trim();
    if (!site) throw new PublishError("Duda: site_name is required", 400);
    const base = apiBase(cfg.region || "us");
